...
vec4 SampleSpecularReflection(vec3 direction, float roughness) {
  ...
  direction = (view * vec4(direction, 0.)).xyz;
  //保证采样到的结果和右手系一致
  #ifdef USE_LEFT_HAND
  direction.z *= -1.;
  #endif
...
vec4 getNormalDistance(vec3 d) {
  //reflection probe生成的cubeRT y轴方向上反了,需要翻转
  // 同时也可以将因为左右手坐标系导致的ny 和 py图像互换和右手保持一致
  vec3 dir = d;
  #ifdef USE_LEFT_HAND
  dir *= vec3(1., -1., 1.);
  #endif

  return textureCube(tCubeMapNormals, dir);
}
...
vec4 SampleSpecularContributionRef(vec3 origin, int i) {
  vec4 ndcPos = projection * view * vec4(origin, 1.);
...
vec4 SampleSpecularContribution(vec3 direction, float roughness) {
  ...
  direction.x *= -1.;
  // 处理手相性 右手坐标系 默认看向-z方向
  #ifndef USE_LEFT_HAND
  direction.z *= -1.;
  #endif
...
vec3 linePlaneIntersect(in vec3 pointOnLine, in vec3 lineDirection, in vec3 pointOnPlane, in vec3 planeNormal) {
  ...
  if(abs(denominator) < EPSILON) {
    return vec3(0.0); // 平行，没有交点
  }
  float t = dot(planeNormal, pointOnPlane - pointOnLine) / denominator;
...
vec3 intersect(vec3 rayOrigin, vec3 rayDirection, inout vec3 hitNormal) {
  ...
  if(sphereHitPoint == vec3(0.0))
    return vec3(0.0); // 如果没有球体相交，直接返回

  vec3 hitPoint = sphereHitPoint;
  vec3 planeNormal = vec3(0.0);

  for(int i = 0; i < 3; ++i) {
    vec3 direction = normalize(hitPoint - CENTER_OFFSET);
    vec4 normalDistanceData = getNormalDistance(direction);
    float distance = normalDistanceData.a * radius;
    vec3 pointOnPlane = CENTER_OFFSET + direction * distance;
    vec3 currentPlaneNormal = getSurfaceNormal(normalDistanceData);

    hitPoint = linePlaneIntersect(rayOrigin, rayDirection, pointOnPlane, currentPlaneNormal);
    planeNormal = currentPlaneNormal; // 每次迭代更新法线
    if(hitPoint == vec3(0.0))
      return vec3(0.0); // 如果没有平面相交，直接返回
...
vec3 getRefractionColor(vec3 origin, vec3 direction, vec3 normal) {
  ...
  // outColor = normalize(newDirection);
  // outColor = origin;
  return outColor;
}
...
vec3 getRefractionColorPoissonSample(vec3 origin, vec3 direction, vec3 normal) {
  ...
  for(int i = 0; i < sampleCount; i++) {
    vec2 offset = poissonDisk[i] * (1. / resolution);
...
void main() {
  ...
  #ifndef IMAGEPROCESSINGPOSTPROCESS
